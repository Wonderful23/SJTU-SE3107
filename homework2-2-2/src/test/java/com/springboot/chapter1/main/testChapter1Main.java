...
public class testChapter1Main extends TestCase{
    ...
    public void mockTestB() throws Exception {
        MvcResult result = this.mockMvc.perform(
                get("/templates")    //请求的url,请求的方法是get
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED).param("temp","hate").param("end","data")         //添加参数
        ).andExpect(status().isOk())    //返回的状态是200
                .andDo(print())         //打印出请求和相应的内容
                .andReturn();// 获取方法的返回值
        String result1 = (String)result.getModelAndView().getModel().get("result");
        Assert.assertEquals("input word is null.", result1);
    }
    ...
    public void mockTestC() throws Exception {
        MvcResult result = this.mockMvc.perform(
                get("/templates")    //请求的url,请求的方法是get
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED).param("begin","").param("end","data")         //添加参数
        ).andExpect(status().isOk())    //返回的状态是200
                .andDo(print())         //打印出请求和相应的内容
                .andReturn();// 获取方法的返回值
        String result1 = (String)result.getModelAndView().getModel().get("result");
        Assert.assertEquals("the input word is invalid", result1);
    }
    @Test
    public void mockTestD() throws Exception {
        MvcResult result = this.mockMvc.perform(
                get("/templates")    //请求的url,请求的方法是get
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED).param("begin","12word").param("end","data")         //添加参数
        ).andExpect(status().isOk())    //返回的状态是200
                .andDo(print())         //打印出请求和相应的内容
                .andReturn();// 获取方法的返回值
        String result1 = (String)result.getModelAndView().getModel().get("result");
